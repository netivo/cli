    ...
    return new Promise((resolve, reject) => {
        let questions = [
            ...
            },
            {
                ...
                name: 'gitRepository',
                message: 'Git repository url (leave blank if you provide this later with git command).',
            }
        ];
        prompt(questions).then(val => {
            ...
            options.git = val.git;
            options.gitRepository = val.gitRepository;
...
export function dev(config = {}) {
    ...
    return new Promise((resolve, reject) => {
        let questions = [
            ...
            {
                type: options.title === '' ? 'text' : null,
                name: 'title',
                message: 'What is the Wordpres title?'
            },
            {
                type: options.host === '' ? 'text' : null,
                name: 'host',
                message: 'What is the SSH host?'
            },
            {
                type: options.dev_domain === '' ? 'text' : null,
                name: 'dev_domain',
                message: 'What is the main domain of the development server?'
            },
            {
                type: options.user === '' ? 'text' : null,
                name: 'user',
                message: 'What is the SSH user?'
            },
            {
                type: options.port === '' ? 'number' : null,
                name: 'port',
                message: 'What is the SSH port?',
            },
            {
                type: options.private_key === '' ? 'text' : null,
                name: 'private_key',
                message: 'What is the path to the private key?'
            }
        ];

        prompt(questions).then(val => {
            if (val.name) options.name = val.name;
            if (val.title) options.title = val.title;
            if (val.host) options.host = val.host;
            if (val.dev_domain) options.dev_domain = val.dev_domain;
            if (val.user) options.user = val.user;
            if (val.port) options.port = val.port;
            if (options.port === '') options.port = 22;
            if (val.private_key) options.private_key = val.private_key;
            resolve(options);
        }).catch(error => {
            reject(error);
        });
...
export function migration(config = {}) {
    ...
    return new Promise((resolve, reject) => {
        let questions = [
            {
                type: options.siteName === '' ? 'text' : null,
                name: 'siteName',
            ...
            {
                type: (config.includeBackup && options.backup_path === '') ? 'text' : null,
                name: 'backup_path',
                message: 'What is the path to the backup file?'
            }
        ];

        prompt(questions).then(val => {
            if (val.siteName) options.siteName = val.siteName;
            if (val.host) options.host = val.host;
            if (val.user) options.user = val.user;
            if (val.port) options.port = val.port;
            if (options.port === '') options.port = 22;
            if (val.private_key) options.private_key = val.private_key;
            if (val.backup_path) options.backup_path = val.backup_path;
            resolve(options);
        }).catch(error => {
            reject(error);
        });
