...
export function metabox() {
    ...
    return new Promise((resolve, reject) => {
        let questions = [
            {
                type: 'text',
                name: 'name',
                message: 'Put the class name with namespace responsive to MetaBox'
            },
            {
                type: 'text',
                name: 'title',
        ...
        prompt(questions).then(val  => {
            ...
            options.name = _.last(n);
            options.path = _.dropRight(n).join('/');
            options.title = val.title;
            options.screen = val.screen;
            options.template = val.template;
            options.context = val.context;
            options.priority = val.priority;
            options.viewName = val.viewName;
            resolve(options);
        }).catch(error => {
            reject(error);
        });
